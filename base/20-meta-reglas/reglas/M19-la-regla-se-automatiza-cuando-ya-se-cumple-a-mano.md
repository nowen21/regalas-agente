...
**Los casos están medidos, y son del propio repositorio.** El ítem 06 del backlog de automatizaciones (la puerta `F2` mecánica) es de prioridad alta y quedó de penúltimo porque, sin las piezas que lo alimentan, su tasa de falsas alarmas lo volvía inservible: la tercera pregunta, descubierta caso por caso en vez de aplicada como regla. Y `F4` reprobó su propio checklist por exigir dos cosas, y de ahí salieron `F4.1` a `F4.5`: de haberse automatizado tal cual, el validador habría congelado la regla doble.
