...
| **Fase** | — | Un pedazo de trabajo que se hace de principio a fin y se guarda | Agente | `documentacion/epicas/EP-NNN-<slug>/HU-NNN-<slug>/<letra>-EP-NNN-HU-NNN-<slug>/` | [`02·F12`](02-flujo-de-trabajo/reglas/F12-relacion-y-nomenclatura-de-fases.md) |
...
| **Dependencia** | — | Cómo se apoya una regla en otra. Solo hay tres maneras: la amplía, la necesita o la reemplaza | Agente, dentro de la regla | En el cuerpo de la regla | [`20·M7`](20-meta-reglas/reglas/M7-las-dependencias-entre-reglas-se-declaran-y-solo-hay-tres.md) |
...
| **Regla** | — | Una sola cosa que hay que hacer, escrita siempre igual y con un ejemplo | Usuario decide, agente redacta | Un archivo por regla, en `reglas/` de su capítulo | [`20·M5`](20-meta-reglas/reglas/M5-toda-regla-se-escribe-en-el-mismo-formato.md) |
...
| **Checklist de despliegue** | Del inglés "lista de chequeo": se recorre marcando antes de sacar el sistema al servidor | La lista de lo que hay que revisar antes y después de sacar el sistema a andar | Agente | Junto al despliegue del proyecto | [`18·DP6`](18-despliegue-e-infraestructura.md#dp6--checklist-de-despliegue) |
...
| **Marco normativo** | — | Qué leyes, normas o auditorías le aplican a este cliente, y qué exige cada una | Usuario del proyecto | `.agente/marco-normativo.md` | [`16 · Cumplimiento y calidad`](16-cumplimiento-y-calidad.md) |
...
| **Postmortem** | En latín, "después de la muerte": se escribe cuando algo ya falló | Lo que se escribe después de que algo falló: qué pasó y qué se cambia para que no vuelva a pasar | Agente | Donde el proyecto guarde sus incidentes | [`19·OB5`](19-observabilidad-y-operacion.md#ob5--postmortem-sin-culpa) |
...
| **Stack** | En inglés, "pila": las tecnologías apiladas una sobre otra | Con qué está hecho el proyecto y dónde vive el código | Usuario del proyecto | `.agente/stack.md` | [`02·F13`](02-flujo-de-trabajo/reglas/F13-deja-la-estructura-base-puesta-antes-de-trabajar.md) |
| **Señal** | Como la señal de una carretera: avisa algo que no se ve desde el código | Algo que se aprendió y que el código no cuenta. Por ejemplo por qué se decidió así, o dónde está la trampa | Agente, al cerrar la unidad | `documentacion/senales.md` y la memoria buscable | [`13·DOC5`](13-documentacion/reglas/DOC5-registra-como-senal-lo-que-no-se-recupera-del-codigo.md) |
