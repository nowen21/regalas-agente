# HU-023 — El capítulo `10` · Dependencias de terceros: su texto tiene dueña
...
| **Módulo / Componente** | Capítulo `10` · Dependencias de terceros |
| **Tipo** | Técnica |
| **Prioridad** | Should |
| **Estimación** | S |
...
- **Quiero** que el texto del capítulo `10` tenga una historia de usuario dueña, que diga de dónde baja cada una de sus reglas y reciba todo cambio de su texto
...
| ID | Regla |
|---|---|
| RN-01 | Todo cambio del texto del capítulo `10` entra como fase de esta historia (`02·F23`) |
...
```gherkin
Dado que el capítulo 10 existe
...
1. Abrir [`base/10-dependencias.md`](../../../../base/10-dependencias.md). Resultado esperado: bajo el título hay una línea «Historia dueña del texto» que enlaza a esta historia.
- **Aprobado cuando:** la línea existe y el enlace resuelve.

### CA-02 — Un cambio del capítulo tiene dónde bajarse

```gherkin
Dado que hay que cambiar el texto de una regla del capítulo 10
...
1. Levantar con el andamio una fase de esta historia para un cambio cualquiera del capítulo (simulado). Resultado esperado: la fase se crea bajo `HU-023-el-capitulo-10-dependencias-de-terceros/`.
...
| Categoría | Requisito |
|---|---|
| Trazabilidad | Se puede nombrar, para el capítulo `10`, la historia donde se escribe su texto |
...
|---|---|---|
| [A-EP-001-HU-023-retrodocumentar-el-capitulo-10](A-EP-001-HU-023-retrodocumentar-el-capitulo-10/) | CA-01, CA-02 | **Terminada** · [Cumple](A-EP-001-HU-023-retrodocumentar-el-capitulo-10/resultado_pruebas.md) |
