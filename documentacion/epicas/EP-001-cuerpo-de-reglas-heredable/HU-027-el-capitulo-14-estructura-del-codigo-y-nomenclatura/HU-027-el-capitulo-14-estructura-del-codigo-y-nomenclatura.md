# HU-027 — El capítulo `14` · Estructura del código y nomenclatura: su texto tiene dueña
...
| **Módulo / Componente** | Capítulo `14` · Estructura del código y nomenclatura |
| **Tipo** | Técnica |
| **Prioridad** | Should |
| **Estimación** | S |
...
- **Quiero** que el texto del capítulo `14` tenga una historia de usuario dueña, que diga de dónde baja cada una de sus reglas y reciba todo cambio de su texto
...
El capítulo [`14 · Estructura del código y nomenclatura`](../../../../base/14-estructura-codigo.md) tiene hoy **3 regla(s)** y, hasta esta historia, ninguna historia de usuario declaraba su texto como módulo: se escribió sin recorrer la cadena que él mismo exige. Lo midió el [pendiente 60](../../../../pendientes/hecho/cada-capitulo-tiene-su-historia.md): 19 de 21 capítulos estaban así, y el usuario decidió una historia por capítulo.
...
| ID | Regla |
|---|---|
| RN-01 | Todo cambio del texto del capítulo `14` entra como fase de esta historia (`02·F23`) |
...
```gherkin
Dado que el capítulo 14 existe
...
1. Abrir [`base/14-estructura-codigo.md`](../../../../base/14-estructura-codigo.md). Resultado esperado: bajo el título hay una línea «Historia dueña del texto» que enlaza a esta historia.
- **Aprobado cuando:** la línea existe y el enlace resuelve.

### CA-02 — Un cambio del capítulo tiene dónde bajarse

```gherkin
Dado que hay que cambiar el texto de una regla del capítulo 14
...
1. Levantar con el andamio una fase de esta historia para un cambio cualquiera del capítulo (simulado). Resultado esperado: la fase se crea bajo `HU-027-el-capitulo-14-estructura-del-codigo-y-nomenclatura/`.
...
| Categoría | Requisito |
|---|---|
| Trazabilidad | Se puede nombrar, para el capítulo `14`, la historia donde se escribe su texto |
...
|---|---|---|
| [A-EP-001-HU-027-retrodocumentar-el-capitulo-14](A-EP-001-HU-027-retrodocumentar-el-capitulo-14/) | CA-01, CA-02 | **Terminada** · [Cumple](A-EP-001-HU-027-retrodocumentar-el-capitulo-14/resultado_pruebas.md) |
...
| Valiosa | Sí | Cierra la trazabilidad hacia arriba de 3 regla(s) |
