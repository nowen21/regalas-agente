# HU-029 — El capítulo `16` · Cumplimiento y calidad: su texto tiene dueña
...
| **Módulo / Componente** | Capítulo `16` · Cumplimiento y calidad |
| **Tipo** | Técnica |
| **Prioridad** | Should |
| **Estimación** | S |
...
- **Quiero** que el texto del capítulo `16` tenga una historia de usuario dueña, que diga de dónde baja cada una de sus reglas y reciba todo cambio de su texto
...
El capítulo [`16 · Cumplimiento y calidad`](../../../../base/16-cumplimiento-y-calidad.md) tiene hoy **0 regla(s)** y, hasta esta historia, ninguna historia de usuario declaraba su texto como módulo: se escribió sin recorrer la cadena que él mismo exige. Lo midió el [pendiente 60](../../../../pendientes/hecho/cada-capitulo-tiene-su-historia.md): 19 de 21 capítulos estaban así, y el usuario decidió una historia por capítulo.
...
| ID | Regla |
|---|---|
| RN-01 | Todo cambio del texto del capítulo `16` entra como fase de esta historia (`02·F23`) |
...
```gherkin
Dado que el capítulo 16 existe
...
1. Abrir [`base/16-cumplimiento-y-calidad.md`](../../../../base/16-cumplimiento-y-calidad.md). Resultado esperado: bajo el título hay una línea «Historia dueña del texto» que enlaza a esta historia.
- **Aprobado cuando:** la línea existe y el enlace resuelve.

### CA-02 — Un cambio del capítulo tiene dónde bajarse

```gherkin
Dado que hay que cambiar el texto de una regla del capítulo 16
...
1. Levantar con el andamio una fase de esta historia para un cambio cualquiera del capítulo (simulado). Resultado esperado: la fase se crea bajo `HU-029-el-capitulo-16-cumplimiento-y-calidad/`.
...
| Categoría | Requisito |
|---|---|
| Trazabilidad | Se puede nombrar, para el capítulo `16`, la historia donde se escribe su texto |
...
| Valiosa | Sí | Cierra la trazabilidad hacia arriba de 0 regla(s) |
