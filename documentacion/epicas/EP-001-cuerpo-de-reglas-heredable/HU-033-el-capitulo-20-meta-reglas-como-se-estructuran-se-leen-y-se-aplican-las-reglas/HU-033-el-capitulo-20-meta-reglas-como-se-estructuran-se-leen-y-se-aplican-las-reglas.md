# HU-033 — El capítulo `20` · Meta-reglas — cómo se estructuran, se leen y se aplican las reglas: su texto tiene dueña
...
| **Módulo / Componente** | Capítulo `20` · Meta-reglas — cómo se estructuran, se leen y se aplican las reglas |
| **Tipo** | Técnica |
| **Prioridad** | Should |
| **Estimación** | S |
...
- **Quiero** que el texto del capítulo `20` tenga una historia de usuario dueña, que diga de dónde baja cada una de sus reglas y reciba todo cambio de su texto
...
El capítulo [`20 · Meta-reglas — cómo se estructuran, se leen y se aplican las reglas`](../../../../base/20-meta-reglas/base.md) tiene hoy **19 regla(s)** y, hasta esta historia, ninguna historia de usuario declaraba su texto como módulo: se escribió sin recorrer la cadena que él mismo exige. Lo midió el [pendiente 60](../../../../pendientes/hecho/cada-capitulo-tiene-su-historia.md): 19 de 21 capítulos estaban así, y el usuario decidió una historia por capítulo.
...
| ID | Regla |
|---|---|
| RN-01 | Todo cambio del texto del capítulo `20` entra como fase de esta historia (`02·F23`) |
...
```gherkin
Dado que el capítulo 20 existe
...
1. Abrir [`base/20-meta-reglas/base.md`](../../../../base/20-meta-reglas/base.md). Resultado esperado: bajo el título hay una línea «Historia dueña del texto» que enlaza a esta historia.
- **Aprobado cuando:** la línea existe y el enlace resuelve.

### CA-02 — Un cambio del capítulo tiene dónde bajarse

```gherkin
Dado que hay que cambiar el texto de una regla del capítulo 20
...
1. Levantar con el andamio una fase de esta historia para un cambio cualquiera del capítulo (simulado). Resultado esperado: la fase se crea bajo `HU-033-el-capitulo-20-meta-reglas-como-se-estructuran-se-leen-y-se-aplican-las-reglas/`.
...
| Categoría | Requisito |
|---|---|
| Trazabilidad | Se puede nombrar, para el capítulo `20`, la historia donde se escribe su texto |
...
|---|---|---|
| [A-EP-001-HU-033-retrodocumentar-el-capitulo-20](A-EP-001-HU-033-retrodocumentar-el-capitulo-20/) | CA-01, CA-02 | **Terminada** · [Cumple](A-EP-001-HU-033-retrodocumentar-el-capitulo-20/resultado_pruebas.md) |
...
| Valiosa | Sí | Cierra la trazabilidad hacia arriba de 19 regla(s) |
