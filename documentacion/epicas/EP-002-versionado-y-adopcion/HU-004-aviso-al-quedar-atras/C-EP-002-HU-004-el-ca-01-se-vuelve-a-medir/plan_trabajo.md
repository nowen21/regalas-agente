# Plan de Trabajo — Fase `C-EP-002-HU-004-el-ca-01-se-vuelve-a-medir` (módulo Enganches de sesión)   ·   `[CAPA 3]`
...
| **HU** | [HU-004](../HU-004-aviso-al-quedar-atras.md) — **una sola** (`F12.1`) |
...
**Objetivo:** comprobar, ejecutando, que **CA-01 hoy se cumple**, y dejarlo declarado donde se lee.

**Fuera de alcance:**

- **Construir nada.** El trabajo ya está hecho; lo que falta es que alguien vuelva a mirarlo.
- **Tocar la fase `A-EP-002-HU-004-retrodocumentar-el-aviso-de-desfase`.** Su veredicto fue cierto el día que se escribió.
...
Dos mitades, y hacen falta las dos: que el aviso **salga** (proyecto temporal que declara una versión vieja) y que el camino de la apertura **pase por él** (`hook_sesion` → `sesion.revisar` → `version.validar`).

**Y por qué la medición no se da por buena de más.** Que el aviso exista no era el problema: ya existía cuando se midió el rojo. Por eso la medición no se da por buena con ver el texto; comprueba el eslabón que faltaba.

**Resultado de la medición:** sale, y `hook_sesion` -> `sesion.revisar` -> `version.validar`
...
| `HU-004-aviso-al-quedar-atras.md` | Modificar | Documentación | Su `Estado` nombra el criterio en rojo |
...
|---|---|---|---|
| CA-01 · El proyecto atrasado recibe el aviso al abrir sesión | Ejecutar el criterio, con su contraprueba | EV-01, EV-02 | ☑ |
