...
| RN-07 · la caja de instrucciones se borra al llenar | doc | Dentro de `DOC20` | ✅ | Texto de la regla |
...
| T-02 | Escribir `DOC19` con su checklist | ✅ hecha | `base/13-documentacion/reglas/DOC19-…` | La regla, checklist en CUMPLE |
| T-03 | Qué es un hueco y qué no | ✅ hecha | Dentro de `DOC19` | Segunda frase del cuerpo |
...
| T-08 | Escribir `DOC20` | ✅ hecha | `base/13-documentacion/reglas/DOC20-…` | La regla |
| T-09 | Escribir `DOC21` | ✅ hecha | `base/13-documentacion/reglas/DOC21-…` | La regla |
...
- **Defectos abiertos que se aceptaron:** DEF-03, el `«ADR-XXX»` de `ADR.md`. Cumple `DOC19` porque el `XXX` va dentro de la marca, como texto. Lo aceptó el agente al verificar y queda anotado.
...
| La sintaxis de un comando no es un hueco | Marcarla haría que el programa de EP-004 reportara de más, que es el riesgo que la épica quiere evitar | Dentro de `DOC19` |
