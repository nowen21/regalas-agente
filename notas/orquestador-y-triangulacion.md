# Pendientes: SDD Orchestrator y triangulación de pruebas
...
| Trazabilidad spec → implementación | Sí | `13-documentacion` · DOC3 |
...
Estación 5: TRAZABILIDAD → puerta: ¿spec cumplida ítem por ítem?
Estación 6: CIERRE       → listo
```

Idea central: **el trabajo no avanza a la siguiente estación si no pasa la puerta de la actual.** No se programa sin spec aprobada; no se cierra con pruebas en rojo.
...
### Decisiones a tomar
- Las **puertas** exactas: ¿spec aprobada? → ¿plan aprobado? → ¿pruebas verdes? → ¿trazabilidad completa? Cada una bloquea el avance.
...
- **Regla nueva en `08-pruebas`** (ej. `T7 · Triangulación`) que defina los métodos concretos:
  ...
  - **Triangulación real:** derivar el resultado esperado desde **2–3 fuentes independientes** (la spec, un cálculo manual, una propiedad invariante) y exigir que coincidan.
- Opcional: **skill `generar-casos-prueba`** que produzca la matriz de corner cases a partir de la spec.
...
**Qué faltaría:**
- El **Task Planner** produce un **grafo**: cada tarea con sus **prerrequisitos** ("esta necesita aquella antes"), no solo una lista.
...
**Esfuerzo:** medio; ligado al Task Planner (ver [`roles-especializados.md`](roles-especializados.md)) y al orquestador.
...
- **Grafo de dependencias entre tareas** → pendiente; parte del Task Planner + el orquestador.
