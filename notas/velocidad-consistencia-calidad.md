...
- **Velocidad cruda:** el **orquestador** (`sdd-orchestrator`) ya está y **dispone** la ejecución en paralelo — usa el grafo de dependencias del Task Planner para correr a la vez las tareas independientes, y cada rol como sub-agente. El paralelo **real** lo ejecuta el entorno (sub-agentes de Claude Code, disponibles).
