...
**6 · Nadie revisó a qué proyectos les borró la memoria el enganche.** El defecto de `recuerdos.py` se corrigió el 2026-08-07 y llega solo a todos los proyectos, porque los enganches llaman al estándar por ruta absoluta. Pero **el arreglo no deshace el borrado anterior**, y no se revisó proyecto por proyecto cuál tenía el enganche ni en qué estado quedó su memoria. En agro-system se recuperó del último commit; de los demás no se sabe.
→ [la memoria del agente en el repositorio · H-3](../historico-chat/resumenes/2026-08-07/memoria-del-agente-en-el-repo.md). **Es el más urgente de esta lista**: lo que no se recupere del commit no está en ninguna parte.
