...
| 90 | **P0** | [Las 650 pruebas de `validadores/tests/` no las corre nada](90-las-pruebas-de-validadores-tests-no-las-corre-nada.md) | Hay dos suites y el repositorio solo conoce una: `pruebas.py` corre sus 515 y la carpeta de 67 archivos no la ejecuta ningún comando — la orden documentada se cae por un `__init__.py` que falta. Corridas a mano: **61 archivos en verde y 6 en rojo**, uno con 98 enlaces mal escritos. Una prueba que nadie corre figura como cobertura, y ya costó seis días de rojo silencioso (`S-075`). |
