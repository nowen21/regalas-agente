...
| ~~19~~ | — | **hecho** → [El capítulo 20 no se cumple a sí mismo](hecho/ninguna-regla-reprueba-su-propio-checklist.md) | De 188 reglas, 129 sin checklist, 7 publicadas en "no cumple" y 33 sin clasificar. Hoy: ninguna sin bloque, ninguna en «no cumple» y ninguna pasada del molde. Cerrado 2026-08-22 (v30.9.1) en la fase `F` de HU-009. |
...
| ~~72~~ | — | **hecho** → [El checklist compara los enganches sensible a mayúsculas de la unidad](hecho/el-checklist-compara-rutas-no-texto.md) | matematica | El validador de la instalación da por faltantes enganches que sí están cuando la letra de unidad de Windows llega con otra capitalización (`c:` contra `C:`). Cualquier proyecto en Windows puede reprobar por eso. |
...
| # | P | Pendiente | Qué resuelve |
|---|---|---|---|
| ~~87~~ | — | **hecho** → [El hash del commit se anota solo](hecho/el-hash-del-commit-se-anota-solo.md) | La plataforma dijo que este repositorio tenía 41 fases abiertas; 23 estaban cerradas de hecho. Se construyó en la `HU-019` con las salidas 1 y 3: un enganche anota el hash al commitear. **Y al medir apareció lo que el pendiente no sabía:** 106 de 140 fases **no tienen la fila** donde marcar, así que no eran 23 sino tres grupos — 22 marcas, 1 trabajo, 106 sin dónde (`S-066`). |

---

### Lo que dejó hacer honesto el número que dice cuánto falta (88)
...
| # | P | Pendiente | Qué resuelve |
|---|---|---|---|
| ~~89~~ | — | **hecho** → [Los guiones de apoyo quedan en el repositorio](hecho/los-guiones-de-apoyo-quedan-en-el-repositorio.md) | La regla se fijó el 2026-08-20, se precisó el 22 y **se dejó de cumplir el 24**: 38 programas en la carpeta temporal del sistema durante cuatro días. El resultado de cada cambio quedaba versionado y el **cómo** se borraba con el temporal. Se construyó en la `HU-018` con las salidas 1 y 3: la regla `04·S18` dice **dónde sí** van, y un enganche avisa al escribir fuera nombrando el destino. Sale de `S-057`. |
...
**El [85](85-las-conversaciones-completas-no-se-pueden-analizar.md) y el [86](86-conectar-un-proyecto-no-tiene-reversa.md) quedaron cerrados el 2026-09-02, y no por una sesión que viniera a cerrarlos.** Los dos pedían cosas que la plataforma construyó como funcionalidades del inventario —`F-033` y `F-034` el uno, `F-035` el otro— el 2026-08-31, y nadie volvió a mirar estos dos archivos. **Un pendiente no se entera de que lo resolvieron**, y por eso se revisan contra lo que ya existe antes de planear trabajo nuevo.
