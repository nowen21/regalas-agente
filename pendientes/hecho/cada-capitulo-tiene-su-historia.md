...
**Estado:** cerrado el 2026-08-22, versión 30.7.0 · anotado 2026-08-17.
...
El usuario eligió la salida **(a): una historia por capítulo** (2026-08-22). Nacieron 21 historias en EP-001, HU-015 a HU-035, una por cada capítulo del `02` al `22` (el `00` y el `01` ya tenían la suya: HU-012 y HU-011). Cada historia es la dueña del **texto** de su capítulo: dos criterios (el capítulo nombra su dueña en la cabecera; un cambio del capítulo tiene dónde bajarse), y su primera fase será la retrodocumentación. Y cada capítulo de `base/`, los 23, declara ahora en su cabecera: «Historia dueña del texto: EP-001 · HU-0NN». Con esto, para cualquier capítulo se puede nombrar la historia donde se escribe su texto, y un cambio de `02·F23` tiene dónde bajarse: HU-015.

Lo que el cierre no hace: las 21 fases de retrodocumentación (una por historia) quedan por ejecutar; el inventario de HU (pendiente 48) las cuenta como incompletas, que es lo correcto.
...
- El [47](el-origen-de-las-reglas-de-negocio.md), que es el mismo hueco un piso más abajo: allá son 31 reglas de negocio que no dicen de dónde bajan, acá es un capítulo entero.
- El [56](el-estandar-tiene-su-planteamiento.md), que es el mismo hueco en la cabeza de la cadena.
