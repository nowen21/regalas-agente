...
**Estado:** **cerrado** el 2026-08-22. Decidido el 2026-08-18, ejecutado el 2026-08-22: las 42 dudas están resueltas en el plan de su fase, y **ninguna fase queda detenida por falta de respuesta**. Anotado el 2026-08-17.
...
| # | Fase | Duda |
|---|---|---|
| 18 | [`A-EP-001-HU-010`](../../documentacion/epicas/EP-001-cuerpo-de-reglas-heredable/HU-010-cuando-no-aplica-la-especificacion/A-EP-001-HU-010-cuando-la-historia-hace-de-especificacion/plan_trabajo.md) | ¿Cuál de los dos caminos del [pendiente 20](cuando-la-historia-hace-de-especificacion.md)? |
...
| 36 | [`A-EP-002-HU-006`](../../documentacion/epicas/EP-002-versionado-y-adopcion/HU-006-quien-sube-la-version/A-EP-002-HU-006-quien-manda-sobre-la-version/plan_trabajo.md) | ¿Cuál de las tres salidas del [pendiente 22](dos-sesiones-versionando-a-la-vez.md)? |
...
| **21** · si la comprobación de marcas cubre todo el repositorio | El [pendiente 11](limpiar-marcadores-de-ia-del-texto-del-estandar.md), paso 3, desde el 2026-08-10 | *«No tocar el histórico: es transcripción literal»*. **Ya está construido así** — se cuenta aparte, y `validar.py marcas` mira solo `base/` y `plantillas/` |
| **18** · cuál de los dos caminos del pendiente 20 | Sigue abierta, pero el [20](cuando-la-historia-hace-de-especificacion.md) ya trae las dos salidas evaluadas | No hace falta pensarla de cero: hay que elegir |
...
| # | Decisión | Por qué |
|---|---|---|
| 18 | **El camino 2 del [pendiente 20](cuando-la-historia-hace-de-especificacion.md):** la historia hace de especificación cuando el entregable no es código | El camino 1 abre una excepción en `F2`, y una excepción es la puerta que después nadie cierra. `08·T1` es el ejemplo vivo |
...
| 36 | **La salida 1 del [pendiente 22](dos-sesiones-versionando-a-la-vez.md):** la versión se sube al guardar, no al editar | La 3 —una sola sesión— ya se incumplió dos veces esta semana. Una regla que la práctica salta no es una regla |
...
### Medido el 2026-08-22: no eran 26 fases detenidas, eran dos cosas distintas

Se contaron los planes cuya §2.7 todavía decía «Pendiente». **Son 31**, y se parten por la mitad:

| | Cuántas | Qué les falta de verdad |
|---|---|---|
| **La fase ya corrió y tiene su veredicto escrito** | 16 | Nada de construcción. La duda se había resuelto y nadie volvió a marcarla |
| **La fase nunca corrió** | 15 | Trabajo de verdad: cada una con su plan, sus pruebas y su cierre |

**Las 16 quedaron al día ese mismo día.** Sus 33 dudas llevan ahora la decisión escrita en la propia §2.7, con su número y su enlace a este pendiente. Nueve de ellas no eran decisiones de nadie: se contestan mirando lo que ya está construido, y quedan marcadas así para que no vuelvan a detener a nadie.

**Y el grupo que este pendiente marcaba como lo más urgente ya no lo es.** El daño vivo del grupo G —una clave pegada en el chat quedaba escrita en claro en la transcripción versionada— está cerrado: existe [`validadores/enmascarar.py`](../../validadores/enmascarar.py) y `historico.py` lo usa antes de escribir.

**Lo que queda son 15 fases:**

| Épica | Fases sin correr |
|---|---|
...
**Cuatro de ellas siguen esperando un dato del usuario** y no una decisión: sobre qué proyecto se prueba, y con qué encargo chico y real. Van con propuesta —**shopnest-mesa**, y **AgroSystem** más **RNI** para la 11—, y una propuesta que nadie corrige se toma por aceptada.
...
**Las 31 fases con la casilla en «Pendiente» se resolvieron todas, y no eran lo que este pendiente decía.** Medidas, se partían por la mitad: **16 ya habían corrido y tenían su veredicto escrito** —la duda estaba resuelta desde el 2026-08-18 y nadie volvió a marcarla— y **15 nunca habían corrido**.

Las 15 se ejecutaron el mismo día. Resultado:
...
**Los cinco «No cumple» son el valor de haberlas corrido**, porque ninguno se sabía:

- Una clave pegada sin comillas queda escrita en claro en la transcripción versionada.
- Una versión adoptada inventada pasa, y si es mayor que la vigente **apaga el aviso de desfase**.
- El aviso de quedarse atrás **no llega al abrir sesión**: hay que pedirlo a mano.
- Las 249 reglas tienen la misma antigüedad de revisión —ninguna—, porque el sello responde por la forma y no por si la regla sigue haciendo falta.
- Un ajuste que contradice el núcleo no se pudo provocar sin escribir en un proyecto real, y eso está prohibido.

**Y las once fases encontraron que su propio plan afirmaba cosas falsas.** No por descuido de quien las escribió: los planes llevaban cinco días quietos mientras el repositorio cambiaba debajo. Es el hallazgo que más lejos llega de todo este pendiente, y quedó como D-01 de la fase [`A-EP-003-HU-007`](../../documentacion/epicas/EP-003-documentos-modelo-y-procedimientos/HU-007-procedimiento-que-dirige/A-EP-003-HU-007-retrodocumentar-el-procedimiento-que-dirige/resultado_pruebas.md): **retomar el hilo y retomar la verdad no son lo mismo**, y el procedimiento solo garantiza lo primero.
