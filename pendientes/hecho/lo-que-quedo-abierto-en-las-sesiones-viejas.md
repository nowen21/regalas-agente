...
**Estado:** ✅ **hecho** el 2026-08-22 (v31.0.0 a v31.4.0) · anotado 2026-08-15 · sale de escribir hacia atrás los resúmenes del [2026-08-06](../../historico-chat/resumenes/2026-08-06/README.md).
...
**1 · ~~El validador de enlaces da por rotos los enlaces con espacios.~~ · CERRADO el 2026-08-17**, junto con el [55](los-enlaces-de-ejemplo-no-son-enlaces.md): `enlaces.py` decodifica el destino antes de buscarlo, y hay dos casos que lo fijan — el archivo con espacios resuelve, y el que de verdad no existe se sigue reportando.

~~**1 · El validador de enlaces da por rotos los enlaces con espacios.**~~ Un enlace a un archivo cuyo nombre lleva espacios se escribe con `%20`, y [`enlaces.py`](../../validadores/enlaces.py) no lo decodifica antes de buscarlo en disco: lo reporta roto aunque el archivo esté. Se arregla con un `unquote` sobre el destino. **Vive en** [EP-004 · HU-005 — Enlaces y citas](../../documentacion/epicas/EP-004-comprobacion-automatica/HU-005-enlaces-y-citas/HU-005-enlaces-y-citas.md), su `RN-01`; se hace junto con el [55](los-enlaces-de-ejemplo-no-son-enlaces.md), que es el mismo falso positivo por el otro lado. El caso que lo destapó salió del repositorio al día siguiente, así que hoy no se ve — pero el validador sigue igual.
...
> **Dónde se atascó, medido el 2026-08-18.** No es que falte trabajo: es que la pieza que falta es **una regla nueva en `base/`**, y por [`02·F23`](../../base/02-flujo-de-trabajo/reglas/F23-ejecuta-un-pendiente-como-fase-de-una-historia-de-usuario.md) eso se construye como fase de su historia. La fase `A` de `EP-001·HU-007` es **retrodocumentación** y excluye escribir reglas nuevas en su propio alcance, así que hace falta una **fase `B`** con su plan aprobado. **Es el mismo sitio donde quedó atascado el [pendiente 16](primero-que-el-proceso-sirva.md)**, que también pide una meta-regla.
>
...
**4 · ~~Renombrar una sesión deja rotos todos los enlaces que la nombran.~~ · CERRADO el 2026-08-17** con el [54](cerrar-un-pendiente-arrastra-sus-citas.md): [validadores/cerrar.py](../../validadores/cerrar.py) arrastra las citas al mover cualquier `.md`, en las dos direcciones. Queda fuera lo que esté fuera del repositorio, que no tiene arreglo desde acá.
...
**El [31](los-resumenes-de-las-sesiones-viejas.md) ya está cerrado**, así que esta lista no crece más por ese lado: son los 33 resúmenes completos. Lo que entre de aquí en adelante sale de sesiones nuevas, no de las viejas.
...
## Las cuatro que pedían trabajo, construidas  ·  2026-08-22

Cada una bajó a fase de su historia y se publicó con su versión, como pide `02·F23`. Ninguna quedó anotada para después.

| Qué faltaba | Qué se construyó | Versión y fase |
|---|---|---|
| El barrido de candidatas sin molde ni disparador | Nace [`20·M20`](../../base/20-meta-reglas/reglas/M20-antes-de-publicar-una-version-se-barre-lo-que-se-pidio-dos-veces.md), que lo exige antes de publicar, y su [molde](../../plantillas/candidatas-a-regla.md). Las cuatro salidas cubren las 27 fichas del barrido que se hizo a mano el 2026-08-13 | v31.0.0 · [`C-EP-001-HU-007`](../../documentacion/epicas/EP-001-cuerpo-de-reglas-heredable/HU-007-regla-de-las-reglas/C-EP-001-HU-007-el-barrido-de-candidatas-tiene-molde-y-disparador/README.md) |
| La dependencia CA→CA en el molde de la historia | La tabla de fases gana la columna «Depende de», que se llena con criterios y vacía es correcta | v31.1.0 · [`B-EP-003-HU-002`](../../documentacion/epicas/EP-003-documentos-modelo-y-procedimientos/HU-002-modelos-del-encargo/B-EP-003-HU-002-la-historia-declara-que-criterio-depende-de-cual/README.md) |
| El mapa del sitio se actualizaba a mano | Nace `validar.py sitio`. **En su primera corrida encontró cuatro carpetas que existían y no estaban en el mapa, y una que el mapa nombraba y ya no existe**: decía doce y son dieciséis | v31.2.0 · [`B-EP-005-HU-011`](../../documentacion/epicas/EP-005-automatismos-que-no-dependen-de-la-memoria/HU-011-donde-termina-el-estandar/B-EP-005-HU-011-el-mapa-del-sitio-tampoco-envejece/README.md) |
| El instalador no completaba el README heredado, y faltaba decir qué manda entre el histórico y lo acordado | `instalar_historico` reusa el mecanismo aditivo del `CLAUDE.md`, sin pisar lo del proyecto; y la plantilla del histórico gana la sección que contesta lo segundo: manda lo acordado, y el histórico no se edita para que cuadre | v31.3.0 · [`B-EP-007-HU-005`](../../documentacion/epicas/EP-007-instalacion-y-actualizacion/HU-005-no-pisar-lo-escrito/B-EP-007-HU-005-el-readme-heredado-recibe-lo-que-la-plantilla-suma/README.md) |
| El índice por temáticas del histórico | Nace `validar.py temas`: **345 hallazgos de 59 resúmenes** en un archivo, cada uno enlazado a su sesión | v31.4.0 · [`C-EP-005-HU-001`](../../documentacion/epicas/EP-005-automatismos-que-no-dependen-de-la-memoria/HU-001-transcripcion-de-la-sesion/C-EP-005-HU-001-el-historico-se-busca-por-tema/README.md) |

**Con esto el pendiente cierra.** Los once puntos que tenía están o cerrados antes, o contestados por el repositorio, o construidos hoy.
