...
**Estado:** cerrado el 2026-08-21 (mismo día), versión 30.3.0.
...
| **De dónde sale** | Pedido del usuario en el chat del 2026-08-21, al analizar el pendiente [73](la-guia-de-entrada-es-del-estandar.md); sus palabras literales en [../prompts/la-administracion-de-proyectos-desde-cimiento.md](../../prompts/la-administracion-de-proyectos-desde-cimiento.md) |
...
El mismo día, en dos frentes. **(a)** `interfaz/` adoptó la estructura de la plantilla Django, completa: `.venv/` propio; `requirements/` con base, local y lock; `config/` con `settings/` (base y local), `wsgi.py` y `asgi.py`; `.env.example`; `templates/` de todo el proyecto en la raíz; `static/` solo con lo propio; el paquete `cimiento/` con un módulo por carpeta (`visor`, `proyectos`), cada uno completo (modelos, admin, formularios, pruebas, migraciones); y los terceros fuera del repositorio: `descargar_estaticos.py` los trae pineados por versión y huella SHA-256 a `terceros/` (que `collectstatic` junta), y el visor sigue funcionando sin internet después de instalado. El usuario revisó la primera versión contra la plantilla y encontró cinco huecos (paquete, `templates/`, `static/vendor`, `asgi`, `.venv`); se cerraron el mismo día. **(b)** Nace el módulo `interfaz/proyectos/`: el registro como modelo de datos con sus pantallas (registrar, editar, dar de baja sin borrar la historia, **medir** el expediente del ciclo de cada proyecto con `validar.py expediente`), con 7 pruebas. `plantillas/proyectos.md` pasó a ser **generado**: la interfaz lo exporta tras cada cambio y el instalador y los avisos lo siguen leyendo; lo que el instalador anote se sube con «importar». Los 10 proyectos reales quedaron en el registro, ida y vuelta verificada.

**Deuda declarada:** el instalador todavía escribe sus altas en el `.md` (la interfaz las importa con un clic); escribirlas directo al registro es mejora futura, no bloqueo.
