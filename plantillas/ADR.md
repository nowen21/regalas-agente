...
> **Architecture Decision Record**: registra una decisión de arquitectura **no obvia** y su porqué (`13·DOC2`), para que no se pierda ni se re-discuta. La produce la estación de diseño (`disenar-arquitectura`) y la referencia la épica (`epica.md §10.2`). Se guarda en `documentacion/adr/ADR-<NNN>-<slug>.md`. Reemplaza los `«…»` y borra esta caja.
