# HU-025 — El capítulo `12` · Privacidad y datos personales: su texto tiene dueña
...
| **Módulo / Componente** | Capítulo `12` · Privacidad y datos personales |
| **Tipo** | Técnica |
| **Prioridad** | Should |
| **Estimación** | S |
...
- **Quiero** que el texto del capítulo `12` tenga una historia de usuario dueña, que diga de dónde baja cada una de sus reglas y reciba todo cambio de su texto
...
El capítulo [`12 · Privacidad y datos personales`](../../../../base/12-privacidad-datos.md) tiene hoy **5 regla(s)** y, hasta esta historia, ninguna historia de usuario declaraba su texto como módulo: se escribió sin recorrer la cadena que él mismo exige. Lo midió el [pendiente 60](../../../../pendientes/hecho/cada-capitulo-tiene-su-historia.md): 19 de 21 capítulos estaban así, y el usuario decidió una historia por capítulo.
...
| ID | Regla |
|---|---|
| RN-01 | Todo cambio del texto del capítulo `12` entra como fase de esta historia (`02·F23`) |
...
```gherkin
Dado que el capítulo 12 existe
...
1. Abrir [`base/12-privacidad-datos.md`](../../../../base/12-privacidad-datos.md). Resultado esperado: bajo el título hay una línea «Historia dueña del texto» que enlaza a esta historia.
- **Aprobado cuando:** la línea existe y el enlace resuelve.

### CA-02 — Un cambio del capítulo tiene dónde bajarse

```gherkin
Dado que hay que cambiar el texto de una regla del capítulo 12
...
1. Levantar con el andamio una fase de esta historia para un cambio cualquiera del capítulo (simulado). Resultado esperado: la fase se crea bajo `HU-025-el-capitulo-12-privacidad-y-datos-personales/`.
...
| Categoría | Requisito |
|---|---|
| Trazabilidad | Se puede nombrar, para el capítulo `12`, la historia donde se escribe su texto |
...
|---|---|---|
| [A-EP-001-HU-025-retrodocumentar-el-capitulo-12](A-EP-001-HU-025-retrodocumentar-el-capitulo-12/) | CA-01, CA-02 | **Terminada** · [Cumple](A-EP-001-HU-025-retrodocumentar-el-capitulo-12/resultado_pruebas.md) |
...
| Valiosa | Sí | Cierra la trazabilidad hacia arriba de 5 regla(s) |
