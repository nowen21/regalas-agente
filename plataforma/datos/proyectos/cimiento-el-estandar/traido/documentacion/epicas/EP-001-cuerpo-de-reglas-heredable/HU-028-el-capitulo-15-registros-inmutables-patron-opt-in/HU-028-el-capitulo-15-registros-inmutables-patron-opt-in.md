# HU-028 — El capítulo `15` · Registros inmutables (patrón opt-in): su texto tiene dueña
...
| **Módulo / Componente** | Capítulo `15` · Registros inmutables (patrón opt-in) |
| **Tipo** | Técnica |
| **Prioridad** | Should |
| **Estimación** | S |
...
- **Quiero** que el texto del capítulo `15` tenga una historia de usuario dueña, que diga de dónde baja cada una de sus reglas y reciba todo cambio de su texto
...
El capítulo [`15 · Registros inmutables (patrón opt-in)`](../../../../base/15-registros-inmutables.md) tiene hoy **7 regla(s)** y, hasta esta historia, ninguna historia de usuario declaraba su texto como módulo: se escribió sin recorrer la cadena que él mismo exige. Lo midió el [pendiente 60](../../../../pendientes/hecho/cada-capitulo-tiene-su-historia.md): 19 de 21 capítulos estaban así, y el usuario decidió una historia por capítulo.
...
| ID | Regla |
|---|---|
| RN-01 | Todo cambio del texto del capítulo `15` entra como fase de esta historia (`02·F23`) |
...
```gherkin
Dado que el capítulo 15 existe
...
1. Abrir [`base/15-registros-inmutables.md`](../../../../base/15-registros-inmutables.md). Resultado esperado: bajo el título hay una línea «Historia dueña del texto» que enlaza a esta historia.
- **Aprobado cuando:** la línea existe y el enlace resuelve.

### CA-02 — Un cambio del capítulo tiene dónde bajarse

```gherkin
Dado que hay que cambiar el texto de una regla del capítulo 15
...
1. Levantar con el andamio una fase de esta historia para un cambio cualquiera del capítulo (simulado). Resultado esperado: la fase se crea bajo `HU-028-el-capitulo-15-registros-inmutables-patron-opt-in/`.
...
| Categoría | Requisito |
|---|---|
| Trazabilidad | Se puede nombrar, para el capítulo `15`, la historia donde se escribe su texto |
...
|---|---|---|
| [A-EP-001-HU-028-retrodocumentar-el-capitulo-15](A-EP-001-HU-028-retrodocumentar-el-capitulo-15/) | CA-01, CA-02 | **Terminada** · [Cumple](A-EP-001-HU-028-retrodocumentar-el-capitulo-15/resultado_pruebas.md) |
...
| Valiosa | Sí | Cierra la trazabilidad hacia arriba de 7 regla(s) |
