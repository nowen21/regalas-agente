# Funcionalidad implementada — Fase `A-EP-001-HU-029-retrodocumentar-el-capitulo-16` (módulo Cuerpo de reglas)   ·   `[CAPA 3]`
...
| Campo | Valor |
|---|---|
| **Fase** (identificador · `02·F12.6`) | `A-EP-001-HU-029-retrodocumentar-el-capitulo-16` |
| **Módulo** | Cuerpo de reglas |
| **Especificación del módulo** | La redacción de los CA de la [HU-029](../HU-029-el-capitulo-16-cumplimiento-y-calidad.md) es la especificación funcional (`02·F19`) |
...
**Nada: se comprobó.** Es retro-documentación (`13·DOC6`). El capítulo `16` ya nombraba su historia dueña en la cabecera; lo que faltaba era **dejarlo verificado y darle a la historia una fase donde bajen sus cambios**.

Con esto, el capítulo `16` y sus **0 reglas** dejan de ser texto sin dueño declarado: tienen historia, y la historia tiene por dónde recibir un cambio.
...
|---|---|---|---|---|
| CA-01 — el capítulo nombra su historia dueña | doc | `base/16-cumplimiento-y-calidad.md`, su cabecera | ✅ | El enlace **resuelve** |
| CA-02 — un cambio tiene dónde bajarse | doc | `HU-029-el-capitulo-16-cumplimiento-y-calidad.md` §8 | ✅ | `validar.py fases` deja de reclamar |
...
| T-01 | La cabecera nombra y el enlace resuelve | ✅ hecha | `base/16-cumplimiento-y-calidad.md` | `CP-001` |
| T-02 | La historia recibe la fila | ✅ hecha | `HU-029-el-capitulo-16-cumplimiento-y-calidad.md` | `CP-002` |
...
- **Verificaciones manuales** (`08·T4`):
  ...
  - El capítulo tiene **0 regla(s)** donde el analizador las ve.
- **Defectos abiertos que se aceptaron:** `D-01` — el analizador no ve las reglas de este capítulo. Su fase existe: `B-EP-004-HU-002`

---

## 4. Cómo se usa / puntos de entrada  ·  `13·DOC1`

- **Punto de entrada:** la cabecera del capítulo `16`. Quien vaya a cambiarlo lee ahí por dónde baja el cambio.
...
|---|---|---|
| **El analizador no ve las reglas de este capítulo**, así que cualquier conteo automático sobre él da cero | Cambio del entorno — el capítulo usa una forma de encabezado que el analizador no reconoce | `B-EP-004-HU-002-el-analizador-ve-todas-las-reglas` |
