# Plan de Trabajo — Fase `A-EP-001-HU-029-retrodocumentar-el-capitulo-16`   ·   `[CAPA 3]`

**Para qué sirve este documento.** Dice **qué se va a hacer en esta fase, en qué orden, sobre qué archivos y cómo se comprueba** cada criterio de aceptación antes de darlo por cumplido. Lo que se pide vive en la [HU-029](../HU-029-el-capitulo-16-cumplimiento-y-calidad.md); con qué casos se comprueba, en el [plan_pruebas.md](plan_pruebas.md).
...
**Dejar comprobado que el capítulo `16` tiene historia dueña declarada, y que un cambio suyo tiene por dónde bajarse.**
...
| Reglas del capítulo `16` | **0** | `metareglas.reglas()`, no contadas a mano |
| Forma del capítulo en el disco | archivo suelto | `retrodocumentar-los-capitulos.py` |
| Fases que tenía la HU-029 antes de esta | **0** | `validar.py fases` |

**Este capítulo aparece con cero reglas, y no es que no tenga: es que el analizador no las ve.** Su encabezado usa una forma que `metareglas.reglas()` no reconoce. Está declarado, no escondido: es lo que arregla la fase `B-EP-004-HU-002-el-analizador-ve-todas-las-reglas`. **No se corrige acá**: esta fase no toca `base/` ni el analizador.

### 2.2 Lo que ya existe y no se rehace

| Pieza | Estado | Qué hace |
|---|---|---|
| El capítulo `base/16-cumplimiento-y-calidad.md` | **Existe** | No se toca |
| Su cabecera con la historia dueña enlazada | **Ya está** | Es el `CA-01`, cumplido |
| La [HU-029](../HU-029-el-capitulo-16-cumplimiento-y-calidad.md) | **Escrita** | Es el `CA-02`: el sitio donde baja un cambio |
...
| `HU-029-el-capitulo-16-cumplimiento-y-calidad.md` §8 | La fila de la fase, y el estado |
...
| T-01 | Comprobar que la cabecera del `16` nombra su historia **y el enlace resuelve** | Test | 0,3 h | T-00 | EV-01 |
| T-02 | Comprobar que la HU-029 existe y su §8 admite la fila | Test | 0,2 h | — | EV-02 |
...
|---|---|---|---|---|
| CA-01 · el capítulo nombra su historia dueña | El programa sobre las 21, y el enlace resuelto | EV-00, EV-01 | Nombra la HU-029 | ☑ |
