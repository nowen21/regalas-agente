# Plan de Trabajo — Fase `D-EP-005-HU-008-el-criterio-de-salida-se-vuelve-a-medir` (módulo Enganche del resumen)   ·   `[CAPA 3]`
...
| **HU** | [HU-008](../HU-008-enganche-del-resumen.md) — **una sola** (`F12.1`) |
...
**Objetivo:** comprobar, ejecutando, que **el criterio de salida hoy se cumple**, y dejarlo declarado donde se lee.

**Fuera de alcance:**

- **Construir nada.** El trabajo ya está hecho; lo que falta es que alguien vuelva a mirarlo.
- **Tocar la fase `A-EP-005-HU-008-enganche-del-resumen`.** Su veredicto fue cierto el día que se escribió.
...
Lo medible se ejecuta: que el enganche esté colgado en `.claude/settings.json`, y que la sesión real haya dejado su resumen con la línea del índice apuntándole después de renombrarla. La mitad manual la atestigua esa sesión.

**Y por qué la medición no se da por buena de más.** Este es el único de los cinco cuyo criterio **un programa no puede firmar solo**: pide una sesión real. Por eso la medición dice qué comprobó y qué atestigua la transcripción, en vez de dar las dos cosas por iguales.

**Resultado de la medición:** colgado, y la sesion real dejo resumen e indice coherentes tras renombrar
...
| `HU-008-enganche-del-resumen.md` | Modificar | Documentación | Su `Estado` nombra el criterio en rojo |
...
|---|---|---|---|
| Criterio de salida · la comprobación en una sesión real | Ejecutar el criterio, con su contraprueba | EV-01, EV-02 | ☑ |
