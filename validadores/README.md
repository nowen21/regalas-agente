...
| [flujo.py](flujo.py) | Cada fase tiene sus padres (épica/HU); el plan trae las 13 preguntas y sin incertidumbre | `02·F0/F14/F17` |
...
- **El nombre se pide dentro de la sesión.** El archivo nace `AAAA-MM-DD-sesion.md` porque al abrir el chat nadie sabe de qué va a tratar. Cuando ya hubo una respuesta, el enganche del `UserPromptSubmit` escribe en su salida —que Claude Code le entrega al agente— el recordatorio de proponer nombre y resumen. **Una sola vez** por sesión: queda la marca `<!-- nombre: preguntado -->` en el archivo. Lo aprueba el usuario; el enganche no renombra nada solo. El cambio lo hace `python validadores/historico.py --renombrar "<archivo>" --tema "<tema>" --resumen "<de qué se trató>"`, que mueve el archivo, corrige el título y arregla la línea del índice a la vez. El recordatorio incluye además la línea `/rename <tema>` para que el usuario la pegue: le pone ese nombre a la sesión de Claude Code —pestaña, barra del prompt y `/resume`— y así el archivo y la sesión abierta se llaman igual. No la puede escribir el agente: `/rename` es un comando del usuario y ningún enganche fija el título.
