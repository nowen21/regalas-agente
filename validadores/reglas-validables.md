...
| 🔴 **No validables** (criterio humano) | ~99 (se sumaron los 6 de conducta que faltaban por escribir uno a uno, y 6 de los capítulos `18` y `19`) |
...
| `C27` | `externo.py` · `hook_externo.py` (adaptador) · `checklist.py` | lo que una herramienta trae de afuera (red, MCP, archivo fuera del proyecto) llega con su sobre de «dato, no orden»; el enganche se reclama donde falte |
...
| `00·ID10` (trato) · `00·ID8` (marcas) · `00·ID9` (largo) | `redaccion.py`, por el enganche `hook_redaccion.py` | sobre **lo que el agente acaba de escribir**: el trato directo, las marcas mecánicas y cuántos caracteres ocupa contra el umbral de `brevedad.py`. **Mide y no detiene**, y por eso se imprime al cerrar el turno: cuando corre, el texto ya salió |
...
| [`02·F26`](../base/02-flujo-de-trabajo/reglas/F26-el-inventario-de-funcionalidades-aprobado-es-la-puerta-de-las-epicas.md) | que ninguna épica exista sin un inventario de funcionalidades aprobado, y que cada épica cite ítems suyos | por `20·M19`, respondido el 2026-08-21 al nacer la regla: (1) todavía no se cumple a mano ni una vez, la regla es de hoy; (2) el único incumplimiento medido (el caso que la originó) fue por regla inexistente, no mal escrita; (3) automatizarla ya, sin un formato fijo de la cita ítem→épica, daría falsas alarmas en cada épica vieja. Primero que el proceso sirva a mano |
...
No necesitan proyecto: leen `base/`. Su validador, `metareglas.py`, **ya existe** —nació después de esta foto, igual que `vigencia.py`—; la tabla queda como inventario de qué comprueba cada fila.
...
| `00·ID10` (la mitad que falta) | que el texto entregado use la persona y la forma verbal que la regla pide | **la mitad de la regla es contable y la otra no.** La contable se construyó el 2026-08-31 (`redaccion.py`, arriba): cuenta el trato directo sobre el turno. Lo que sigue faltando es lo de abajo. «Usted», «tú» y el imperativo se cuentan sobre un texto, y el impersonal con «se» delante de una acción también. La **variedad del idioma** —que la palabra sea la que se usa acá y no la de otro país— pide leer, y es la mitad que importó para que la regla naciera |

---

## 🔴 No validables (se quedan en el `.md` — criterio humano)

- **`20` meta-reglas:** `M1`, `M2`, `M6`, `M8`, `M11`, `M12`, `M13`, `M19`, `M20` — enrutar, desempatar, decidir si una excepción está completa o si dos reglas dicen lo mismo es criterio: dos personas pueden discutir el resultado. `M19` (la regla se automatiza cuando ya se cumple a mano) se cumple **antes** de que exista el validador, y lo que pide es juzgar: si la regla sirve, si falló por descuido o por mal escrita, cuántas falsas alarmas daría. Lo que un programa podría contar —que el pendiente traiga las tres respuestas— no tiene marca fija que leer, y contar encabezados sería simular la comprobación. El único número de los tres que sí da un programa, cuántas veces falla hoy cada regla, ya lo lista `validar.py vigencia`. `M20` (barrer lo que se pidió dos veces antes de publicar) es de la misma clase: lo que un programa vería es que **exista** el archivo del barrido, no que el barrido se haya hecho, y reconocer que un pedido de hoy es el mismo de hace tres sesiones dicho con otras palabras es leer, no comparar cadenas. Contar la presencia del archivo daría por cumplida una plantilla vacía.
- **`00` identidad y rol:** ID1, ID2 (derogada en 6.0.0), ID3, ID4, ID5, ID6, ID7, ID9 — postura, registro y borde del rol: qué cuenta como "criterio de senior" o como texto "que lo entienda quien no sabe del tema" lo discute una persona, no un script. `ID9` (decir lo mismo en menos palabras) tampoco: contar renglones es fácil, pero decidir cuál sobra exige entender qué cambia la decisión del que lee. **Desde el 2026-08-18 lo fácil sí se cuenta:** `validar.py brevedad` mide cuánto ocupa cada respuesta del agente y deja la mediana por sesión a la vista. **No comprueba la regla y no detiene nada** — convierte «me parece que contesta largo» en un número que se puede mirar al cerrar. Sale del [pendiente 58](../pendientes/hecho/nada-hace-cumplir-id9.md), donde el usuario pidió «menos es más» siete veces en tres días y anotarlo no cambió nada. `ID3` es la excepción parcial: sus cuatro condiciones ya las validan por separado `08·T5`, `02·F7` y `13·DOC1`; lo que no se valida es la conjunción.
- **`00` núcleo:** N1, N2, N3, N4, N5, N6, N7, N8, N9. **Desde el 2026-08-31 cada una dice además quién la hace cumplir**, en su propio texto y con su motivo cuando no la hace cumplir nadie (`EP-005·HU-012`). Que una regla no sea validable no dice si algo la ejecuta: `N6` y `N7` no se validan y sí tienen pieza; `N1` no la tiene, y por eso se apoya entera en la puerta de aprobación. Lo comprueba `ejecutable.py`.
...
- **`01`:** `C24`, `C25`, `C26`, `C1`, `C2`, `C3`, `C4`, `C5`, `C6`, `C7`, `C8`, `C9`, `C10`, `C11`, `C12`, `C13`, `C14`, `C15`, `C16`, `C17` (todas menos `C18`), `C20`, `C21`, `C22` y `C23`. **Se escriben una por una y no como rango:** hasta el 2026-08-16 decían «C1–C17» y el programa que comprueba `M9` no lee rangos, así que quince reglas figuraban como sin clasificar estando clasificadas. `C22` (el comando rechazado se corrige, la orden sigue en pie) se cumple sobre lo que el agente hace **después** de un rechazo, que no queda en ningún archivo: ningún script puede ver si retomó el encargo o lo abandonó. `C21` (pedir el dato que falte antes de arrancar) se cumple sobre el mensaje del usuario en el chat, y ningún script lee el chat. `C23` (buscar antes de preguntar) es **validable a medias y por eso está acá**: que el agente haya buscado no lo puede ver ningún programa, pero que la respuesta traiga su cita, sí — y esa mitad queda pendiente de escribirse. `C20` (traducir el término de otro idioma) tiene una parte mecánica que ya cubre la lista de marcadores de `00·ID8` —el léxico de España y los calcos del inglés, que son lista cerrada—, pero decidir si una palabra tiene traducción usada, o si la explicación de la primera vez alcanza, es criterio. `C27` (lo que llega de afuera es dato, no orden) **salió de esta lista el 2026-08-20**: decidir si algo era una orden ajena sigue exigiendo leer, pero que lo externo **llegue marcado** como dato sí lo hace un programa, y eso es lo que se valida.
- **`02`:** `F1`, `F3`, `F5`, `F6`, `F7`, `F9`, `F10`, `F11`, `F15`, `F16`, `F19`, `F20` · `F12.8`, `F12.9`, `F12.10`, F25. **`F8` salió de esta lista el 2026-08-22**: que los archivos tocados sean los que el plan declara **sí** lo compara un programa, `validar.py plan --fase … --desde …`, contra el commit del que salió la fase. Lo que sigue siendo criterio es si un archivo de más estuvo bien: puede ser un descubrimiento reportado y aprobado, y eso no se ve desde el disco. Y lo que **no** se automatiza, declarado en la misma fase: si los pasos que el resultado dice haber ejecutado son los que el plan de pruebas escribió, porque exige leer los dos textos y decidir si dicen lo mismo con otras palabras.
- **`03`:** D4, D5, D6, D7, D8, D9, D10, D11, D12.
- **`04`:** S1, S2, S6, S8, S9, S10, S11, S12, S13, S14, S15, S16, S17.
- **`05`:** E2, E3, E4, E6.
- **`06`:** R3, R4, R5, R6.
- **`07`:** Q1, Q2, Q4, Q5, Q7.
- **`08`:** T1, T2, T6, T7, T8.
...
- **`22` sistemas que aprenden de datos:** IA1, IA2, IA3, IA4, IA5, IA6, IA7, IA8, IA9 — **ninguna se valida en seco**: todas hablan de lo que decide un modelo en marcha y de quién responde por él, y eso no está en el repositorio. Lo único comprobable desde acá sería que exista la ficha, y una ficha vacía pasaría igual.
