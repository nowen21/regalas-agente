...
- **`20` meta-reglas:** `M1`, `M2`, `M6`, `M8`, `M11`, `M12`, `M13`, `M19` — enrutar, desempatar, decidir si una excepción está completa o si dos reglas dicen lo mismo es criterio: dos personas pueden discutir el resultado. `M19` (la regla se automatiza cuando ya se cumple a mano) se cumple **antes** de que exista el validador, y lo que pide es juzgar: si la regla sirve, si falló por descuido o por mal escrita, cuántas falsas alarmas daría. Lo que un programa podría contar —que el pendiente traiga las tres respuestas— no tiene marca fija que leer, y contar encabezados sería simular la comprobación. El único número de los tres que sí da un programa, cuántas veces falla hoy cada regla, ya lo lista `validar.py vigencia`.
